...
MIN_MTF_ALIGNMENT_PCT = float(os.environ.get("MIN_MTF_ALIGNMENT_PCT", "50.0"))
MIN_SMC_SCORE         = int(os.environ.get("MIN_SMC_SCORE", "3"))

# Minimum final confidence required to send a signal (after filters)
MIN_CONFIDENCE_FOR_SIGNAL = float(os.environ.get("MIN_CONFIDENCE_FOR_SIGNAL", "65.0"))
...
class HybridPortfolioSystemV3:
    ...
    async def _apply_confirmation_filters(
        ...
        if filters_passed >= 1:
            # +8% per filter passed
            final_confidence = min(99.0, confidence + (filters_passed * 8.0))

        if filters_passed == 3:
            # All filters passed → guarantee 95%+ confidence
            final_confidence = min(99.0, max(95.0, final_confidence))

        # Downgrade to NEUTRAL only if ALL 3 filters fail (0/3)
        final_signal = signal if filters_passed >= 1 else "NEUTRAL"
