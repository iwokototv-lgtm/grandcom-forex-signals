...
# ============================================================
# EXECUTION GATEKEEPER  (Safe Execution Mode)
# Implements all rules from Safe_Execution_Mode_Plan.docx
# ============================================================
...
class _MarketCondition(str, Enum):
    TRENDING = "TRENDING"
    RANGING  = "RANGING"
    UNKNOWN  = "UNKNOWN"
...
class GatekeeperConfig:
    MIN_RR_RATIO:          float = float(os.getenv("GK_MIN_RR_RATIO",          "1.5"))
    MAX_SIGNAL_AGE_SECONDS:float = float(os.getenv("GK_MAX_SIGNAL_AGE_S",      "5"))
    MAX_SLIPPAGE:          float = float(os.getenv("GK_MAX_SLIPPAGE",           "0.0003"))
    MAX_SPREAD:            float = float(os.getenv("GK_MAX_SPREAD",             "0.0005"))
    EMA50_PROXIMITY_PCT:   float = float(os.getenv("GK_EMA50_PROXIMITY_PCT",    "0.5"))
    MAX_OPEN_TRADES:       int   = int(os.getenv("GK_MAX_OPEN_TRADES",          "3"))
    BLOCK_RANGE_MARKETS:   bool  = os.getenv("GK_BLOCK_RANGE_MARKETS", "true").lower() == "true"
    LOG_FILE: Optional[str]      = os.getenv("GK_LOG_FILE", "gatekeeper_trades.jsonl")

_gk_logger = logging.getLogger("execution_gatekeeper")

def _gk_log(pair: str, signal_type: str, approved: bool, code: str, reason: str):
    status = "APPROVED" if approved else "REJECTED"
    _gk_logger.info("%s %s %s [%s] %s", status, pair, signal_type, code, reason)
    if GatekeeperConfig.LOG_FILE:
        try:
            entry = {
            ...
            with open(GatekeeperConfig.LOG_FILE, "a", encoding="utf-8") as fh:
                fh.write(_json.dumps(entry) + "\n")
        except OSError:
            pass

def _check_direction(signal_type: str, entry: float, tp: float, sl: float) -> _GKResult:
    ok = False
    if signal_type == "BUY":
        ok = tp > entry > sl
        ...
        ok = tp < entry < sl
    if not ok:
        return _GKResult(False, "DIRECTION_INVALID",
            f"{signal_type} TP/SL placement wrong: entry={entry}, TP={tp}, SL={sl}")
    return _GKResult(True)

def _check_rr(entry: float, tp: float, sl: float) -> _GKResult:
    risk   = abs(entry - sl)
    reward = abs(tp - entry)
    if risk == 0:
        return _GKResult(False, "RR_ZERO_RISK", "SL equals entry — undefined R:R")
    rr = reward / risk
    if rr < GatekeeperConfig.MIN_RR_RATIO:
        return _GKResult(False, "RR_INSUFFICIENT",
            f"R:R {rr:.2f} < minimum {GatekeeperConfig.MIN_RR_RATIO:.2f}")
    return _GKResult(True)

def _check_latency(signal_ts: float) -> _GKResult:
    age = time.time() - signal_ts
    if age > GatekeeperConfig.MAX_SIGNAL_AGE_SECONDS:
        return _GKResult(False, "SIGNAL_STALE",
            f"Signal is {age:.1f}s old > max {GatekeeperConfig.MAX_SIGNAL_AGE_SECONDS}s")
    return _GKResult(True)

def _check_slippage(entry: float, current: float) -> _GKResult:
    slip = abs(current - entry)
    if slip > GatekeeperConfig.MAX_SLIPPAGE:
        return _GKResult(False, "SLIPPAGE_EXCEEDED",
            f"Slippage {slip:.5f} > max {GatekeeperConfig.MAX_SLIPPAGE:.5f}")
    return _GKResult(True)

def _check_spread(spread: float) -> _GKResult:
    if spread > GatekeeperConfig.MAX_SPREAD:
        return _GKResult(False, "SPREAD_TOO_HIGH",
            f"Spread {spread:.5f} > max {GatekeeperConfig.MAX_SPREAD:.5f}")
    return _GKResult(True)

def _check_entry_quality(entry: float, ema50: float) -> _GKResult:
    if ema50 <= 0:
        return _GKResult(True)  # no EMA data — skip
    pct = abs(entry - ema50) / ema50 * 100
    if pct > GatekeeperConfig.EMA50_PROXIMITY_PCT:
        return _GKResult(False, "ENTRY_QUALITY_POOR",
            f"Entry {pct:.2f}% away from EMA50 (max {GatekeeperConfig.EMA50_PROXIMITY_PCT}%)")
    return _GKResult(True)

def _check_range_market(is_ranging: bool, pair: str) -> _GKResult:
    if GatekeeperConfig.BLOCK_RANGE_MARKETS and is_ranging:
        return _GKResult(False, "MARKET_RANGING",
            f"{pair} is RANGING — trading disabled in range markets")
    return _GKResult(True)

def _check_open_trades(open_count: int) -> _GKResult:
    if open_count >= GatekeeperConfig.MAX_OPEN_TRADES:
        return _GKResult(False, "MAX_OPEN_TRADES",
            f"{open_count} open trades >= limit {GatekeeperConfig.MAX_OPEN_TRADES}")
    return _GKResult(True)
...
def run_execution_gatekeeper(
    ...
    spread:      float,
    ema50:       float,
    signal_ts:   float,   # unix timestamp when signal was generated
    is_ranging:  bool,
    open_trades: int,
) -> tuple[bool, str, str]:
    """
    ...
    Logs every decision to gatekeeper_trades.jsonl.
    """
    checks = [
        _check_direction(signal_type, entry_price, tp1, sl_price),
        _check_rr(entry_price, tp1, sl_price),
        _check_latency(signal_ts),
        _check_slippage(entry_price, current_price),
        _check_spread(spread),
        _check_entry_quality(entry_price, ema50),
        _check_range_market(is_ranging, pair),
        _check_open_trades(open_trades),
    ]
    for chk in checks:
        if not chk.approved:
            _gk_log(pair, signal_type, False, chk.reason_code, chk.reason)
            return False, chk.reason_code, chk.reason

    _gk_log(pair, signal_type, True, "OK", "All checks passed")
...
PAIR_PARAMETERS = {
    "XAUUSD": {
        "enabled": True,
        "use_fixed_pips": True,
        "fixed_tp1_pips": 7, "fixed_tp2_pips": 15, "fixed_tp3_pips": 25,
        "atr_multiplier_sl": 1.5, "min_rr": 1.5,
        "pip_value": 0.1, "decimal_places": 2, "typical_spread": 0.30
    },
    "XAUEUR": {
        "enabled": True,
        "use_fixed_pips": True,
        "fixed_tp1_pips": 5, "fixed_tp2_pips": 10, "fixed_tp3_pips": 15,
        "atr_multiplier_sl": 1.5, "min_rr": 1.5,
        "pip_value": 0.1, "decimal_places": 2, "typical_spread": 0.40
    },
    "BTCUSD": {
        "enabled": False,
...
GOLD_CONFIDENCE_THRESHOLD = 75
SIGNAL_THROTTLE_MINUTES = 45
SESSION_FILTERS = {}
DRAWDOWN_PROTECTION = {
    "enabled": True,
    "max_daily_losses": 3,
...
async def generate_signal_for_pair(pair: str) -> Optional[Signal]:
    ...
    try:
        ...
        if ai_confidence < effective_threshold:
            logger.info(f"📊 {pair} skipped - confidence {ai_confidence}% < {effective_threshold}%")
            return None
        ...
        # ================================================================
        # EXECUTION GATEKEEPER  ← NEW VALIDATION LAYER
        # All rules from Safe_Execution_Mode_Plan.docx enforced here.
        # ================================================================
        try:
            open_trades_count = await db.signals.count_documents({"status": "ACTIVE"})
            spread = params.get("typical_spread", 0.0002)
            ema50  = indicators.get("ema_50", 0.0)
            is_ranging = (regime_name == "RANGE")

            gk_approved, gk_code, gk_reason = run_execution_gatekeeper(
                pair          = pair,
                signal_type   = ai_analysis["signal"],
                entry_price   = entry_price,
                tp1           = tp_levels[0] if tp_levels else entry_price,
                sl_price      = sl_price,
                current_price = indicators["current_price"],
                spread        = spread,
                ema50         = ema50,
                signal_ts     = signal_generated_at,
            ...
            logger.info(f"✅ GATEKEEPER APPROVED {pair} {ai_analysis['signal']}")

        except Exception as gk_err:
            # Log but don't block — gatekeeper errors should not crash signal generation
        ...
        signal = Signal(
            ...
            is_premium   = adjusted_confidence > 70
        )
...
async def send_signal_to_telegram(signal: Signal, regime_name: str = "UNKNOWN", risk_mult: float = 1.0):
    try:
        ...
        regime_emoji = {"TREND_UP": "📈", "TREND_DOWN": "📉", "RANGE": "↔️", "HIGH_VOL": "⚡"}.get(regime_name, "📊")
...
<i>🤖 Powered by Grandcom ML Engine v2 + Safe Execution Mode</i>
        """
...
async def _run_single_pair_backtest(pair, request, start_dt, end_dt, engine, user_id):
    ...
    result_doc = {"user_id": user_id, "pair": pair, "enabled": is_enabled,
        ...
        "filters_applied": {"use_pair_parameters": request.use_pair_parameters,
            ...
            "gatekeeper": {"min_rr": GatekeeperConfig.MIN_RR_RATIO,
                           "max_signal_age_s": GatekeeperConfig.MAX_SIGNAL_AGE_SECONDS,
                           "max_slippage": GatekeeperConfig.MAX_SLIPPAGE,
                           "max_spread": GatekeeperConfig.MAX_SPREAD,
                           "block_range_markets": GatekeeperConfig.BLOCK_RANGE_MARKETS}},
...
async def get_backtest_summary(current_user: dict = Depends(get_current_user)):
    try:
        ...
        return {"success": True, "total_pairs_configured": len(PAIR_PARAMETERS),
            "gatekeeper_settings": {"min_rr": GatekeeperConfig.MIN_RR_RATIO,
                "max_signal_age_s": GatekeeperConfig.MAX_SIGNAL_AGE_SECONDS,
                "block_range_markets": GatekeeperConfig.BLOCK_RANGE_MARKETS,
                "max_open_trades": GatekeeperConfig.MAX_OPEN_TRADES},
...
async def admin_create_signal(signal: ManualSignalRequest, admin_user: dict = Depends(require_admin)):
    try:
        ...
        open_count = await db.signals.count_documents({"status": "ACTIVE"})
        params = PAIR_PARAMETERS.get(signal.pair, DEFAULT_PAIR_PARAMS)
        gk_approved, gk_code, gk_reason = run_execution_gatekeeper(
            pair=signal.pair, signal_type=signal.type,
            entry_price=signal.entry_price, tp1=signal.tp1, sl_price=signal.sl,
            current_price=signal.entry_price, spread=params.get("typical_spread",0.0002),
            ema50=0.0, signal_ts=time.time(), is_ranging=False, open_trades=open_count
        )
        if not gk_approved:
...
async def get_profitability_filters(admin_user: dict = Depends(require_admin)):
    return {"success": True, "filters": {
        ...
        "execution_gatekeeper": {"min_rr_ratio": GatekeeperConfig.MIN_RR_RATIO,
            "max_signal_age_seconds": GatekeeperConfig.MAX_SIGNAL_AGE_SECONDS,
            "max_slippage": GatekeeperConfig.MAX_SLIPPAGE, "max_spread": GatekeeperConfig.MAX_SPREAD,
            "ema50_proximity_pct": GatekeeperConfig.EMA50_PROXIMITY_PCT,
            "max_open_trades": GatekeeperConfig.MAX_OPEN_TRADES,
            "block_range_markets": GatekeeperConfig.BLOCK_RANGE_MARKETS,
            "log_file": GatekeeperConfig.LOG_FILE,
...
async def get_filter_statistics(admin_user: dict = Depends(require_admin)):
    ...
    return {"success": True,
        ...
        "gatekeeper_log": GatekeeperConfig.LOG_FILE}
...
async def get_system_config(admin_user: dict = Depends(require_admin)):
    ...
    return {"success": True, "config": {
        ...
        "execution_gatekeeper": {"enabled":True,
            "min_rr_ratio": GatekeeperConfig.MIN_RR_RATIO,
            "max_signal_age_seconds": GatekeeperConfig.MAX_SIGNAL_AGE_SECONDS,
            "max_slippage": GatekeeperConfig.MAX_SLIPPAGE,
            "max_spread": GatekeeperConfig.MAX_SPREAD,
            "ema50_proximity_pct": GatekeeperConfig.EMA50_PROXIMITY_PCT,
...
async def startup_event():
    ...
    logger.info(f"✅ Execution Gatekeeper active — min R:R={GatekeeperConfig.MIN_RR_RATIO}, "
                f"max age={GatekeeperConfig.MAX_SIGNAL_AGE_SECONDS}s, "
                f"max open trades={GatekeeperConfig.MAX_OPEN_TRADES}, "
                f"block ranging={GatekeeperConfig.BLOCK_RANGE_MARKETS}")
