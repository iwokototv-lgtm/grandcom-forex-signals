"""
Model Rescanner — 4H Momentum & Regime Monitor
===============================================
Runs every 4 hours (cron: 0 */4 * * *) to detect momentum shifts,
volatility extremes, and DXY correlation conflicts across all 23 pairs.
...
Actions taken on regime change:
  ...
  - Sends Telegram alert with before/after regime
...
DXY_ROLE: Dict[str, str] = {
    "USDJPY": "USD_LEAD",   "USDCAD": "USD_LEAD",   "USDCHF": "USD_LEAD",
    "EURUSD": "USD_FOLLOW", "GBPUSD": "USD_FOLLOW", "AUDUSD": "USD_FOLLOW",
    "NZDUSD": "USD_FOLLOW",
    "EURJPY": "CROSS_JPY",  "GBPJPY": "CROSS_JPY",  "AUDJPY": "CROSS_JPY",
    "CADJPY": "CROSS_JPY",  "CHFJPY": "CROSS_JPY",
    "EURGBP": "CROSS",      "EURCHF": "CROSS",       "EURAUD": "CROSS",
    "EURCAD": "CROSS",      "GBPCAD": "CROSS",       "GBPAUD": "CROSS",
    "AUDNZD": "CROSS",
    "XAUUSD": "USD_FOLLOW", "XAUEUR": "CROSS",
}
...
def classify_dxy_conflict(
    ...
    Logic:
      USD_LEAD  pairs (USDJPY, USDCAD, USDCHF):
        DXY BULLISH → pair should be BULLISH; conflict if pair is BEARISH
      USD_FOLLOW pairs (EURUSD, GBPUSD, AUDUSD, NZDUSD, XAUUSD):
        DXY BULLISH → pair should be BEARISH; conflict if pair is BULLISH
      CROSS / CROSS_JPY / XAUEUR:
    ...
    if role == "USD_LEAD":
        # Strong DXY → USD pairs should be BULLISH
        if dxy_regime == REGIME_BULLISH and pair_regime == REGIME_BEARISH:
            return True
        if dxy_regime == REGIME_BEARISH and pair_regime == REGIME_BULLISH:
            return True
...
async def send_telegram(bot: Optional[Bot], message: str) -> None:
    """
    Send a plain-text Telegram message with basic retry on flood control.
    Silently skips if bot is not configured.
    """
    if bot is None:
...
async def process_pair(
    ...
    """
    Full 4H rescan for a single pair.
...
async def run_rescanner() -> None:
    ...
    3. Fetch DXY regime (used for correlation checks across all pairs)
    4. Process all 23 pairs sequentially (rate-limit friendly)
    5. Send summary alert
    """
    start_time = datetime.now(timezone.utc)
    logger.info("=" * 60)
    logger.info("🔍 4H Model Rescanner starting")
    logger.info(f"   Pairs: {len(ALL_PAIRS)} | DB: {DB_NAME}")
    logger.info("=" * 60)
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        logger.info("Fetching DXY 4H candles for correlation baseline...")
        ...
        if dxy_df is not None:
            ...
            logger.warning("DXY data unavailable — correlation checks skipped")
    ...
    logger.info("=" * 60)
    logger.info(f"✅ Rescanner complete in {elapsed:.1f}s")
    ...
    # ── Cleanup ───────────────────────────────────────────────────────────────
    mongo_client.close()
    logger.info("MongoDB connection closed. Rescanner done.")
