...
async def check_breakeven():
    """
    Runs every 5 minutes. For every ACTIVE gold signal:
    - Fetches current live price
    - If price has reached TP1 AND breakeven not yet triggered:
        → Updates DB: breakeven_triggered=True, sl_price=entry_price
        → Sends Telegram notification to move SL to entry
    This protects profit on TP2 and TP3 runners.
    """
    try:
        ...
        for sig in active:
            ...
            if TELEGRAM_BOT_TOKEN:
                ...
                be_message = (
                    f"\U0001f514 BREAKEVEN \u2014 {pair} {direction}\n"
                    f"\n"
                    f"\u2705 TP1 hit @ {tp1}\n"
                    f"\U0001f4cc Move SL \u2192 {entry_price} (entry = breakeven)\n"
                    f"\n"
                    f"Remaining targets:\n"
                    f"TP2: {tp2}\n"
                    f"TP3: {tp3}\n"
                    f"\n"
                    f"\U0001f6e1 Trade is now risk-free\n"
                    f"\u23f0 {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}\n"
                    f"Grandcom Gold EA\n"
                )
                try:
                    await bot.send_message(chat_id=TELEGRAM_GOLD_CHANNEL, text=be_message)
...
async def generate_gold_signal(pair: str):
    try:
        ...
        signal_doc = {
            ...
            "atr":                 indicators["atr"],
            "status":              "ACTIVE",
            "breakeven_triggered": False,   # set True when TP1 hit
...
async def lifespan(app: FastAPI):
    ...
    # Breakeven monitor — checks every 5 minutes
    scheduler.add_job(
        check_breakeven, "interval",
        minutes=5,
    ...
    logger.info(
        ...
        f"   Breakeven:  Monitors every 5 min, triggers on TP1 hit\n"
    )
...
async def get_breakeven_signals():
    """
    MT5 EA polls this endpoint every minute.
    Returns signals where TP1 was hit and SL should be moved to entry.
    The EA uses this to update open trades on the MT5 platform.
    """
    signals = await db.gold_signals.find(
        {"status": "ACTIVE", "breakeven_triggered": True},
        {"_id": 0, "pair": 1, "type": 1, "entry_price": 1,
         "tp_levels": 1, "sl_price": 1, "breakeven_price": 1, "breakeven_at": 1}
