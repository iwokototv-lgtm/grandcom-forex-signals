...
# All new features appended below existing code
# ============================================================
...
SESSION_CONFIDENCE = {
    "LONDON_NY_OVERLAP": {"hours": (12, 16), "min_score": 60},   # Best — normal threshold
    "LONDON":            {"hours": (7,  12), "min_score": 65},   # Good — slightly higher
    "NEW_YORK":          {"hours": (16, 21), "min_score": 65},   # Good
    "ASIAN":             {"hours": (0,   7), "min_score": 78},   # Thin — much higher bar
    "DEAD_ZONE":         {"hours": (21, 24), "min_score": 85},   # Late NY/early Asian — very strict
}

# ============================================================
# GATE: BLACK BOX LOGGING (Feature 21, 26)
# ============================================================
async def log_blackbox(
    ...
    """
    Institutional-grade black box logger.
    Every signal attempt (pass or block) is written to JSONL.
    Enables post-mortem analysis of which gates fire most.
    """
    try:
        entry = {
            ...
            "decision":       decision,   # PASSED / BLOCKED
        ...
        # Write CSV denial log (blocked only)
        if decision == "BLOCKED":
...
# ============================================================
# GATE: FLASH CRASH CIRCUIT BREAKER (Features 8i, 14i)
# ============================================================
async def check_circuit_breaker(pair: str, current_price: float) -> tuple[bool, str]:
    """
    Halts trading if price moves >$15 in <2 minutes (flash crash).
    Pauses for 30 minutes to wait for spread stabilisation.
    """
...
# ============================================================
# GATE: SESSION-BASED CONFIDENCE FILTER (Feature 13)
# ============================================================
def get_session_min_score() -> tuple[int, str]:
    """
    Returns the minimum tech score required for the current UTC hour.
    Higher requirements during thin market sessions.
    """
...
# ============================================================
# GATE: SHANNON ENTROPY FILTER (Feature 11)
# ============================================================
def calculate_shannon_entropy(df: pd.DataFrame, window: int = 20) -> tuple[bool, float, str]:
    """
    Calculates Shannon Entropy on price returns.
    High entropy = random/chaotic market → suppress trading.
    Low entropy  = organised/trending market → allow trading.
    Returns (is_high_entropy, entropy_value, reason)
    """
    try:
        if len(df) < window + 5:
        ...
        # Max possible entropy for 10 bins = log2(10) ≈ 3.32
        max_entropy    = np.log2(10)
        entropy_ratio  = entropy / max_entropy

        is_chaotic = entropy_ratio > 0.85   # >85% of max entropy = chaotic
        reason     = (
...
# ============================================================
# GATE: FAIR VALUE GAP DETECTION (Feature 12)
# ============================================================
def detect_fair_value_gaps(df: pd.DataFrame, lookback: int = 20) -> dict:
    """
    Detects Fair Value Gaps (FVG) — price imbalances left by fast moves.
    A bullish FVG: low[i+1] > high[i-1] (gap up)
    A bearish FVG: high[i+1] < low[i-1]  (gap down)
    Returns nearest FVG and its distance from current price.
    """
    try:
        ...
        for i in range(1, len(recent) - 1):
            ...
            mid_close = float(recent.iloc[i]["close"])
...
# ============================================================
# GATE: HURST EXPONENT — REGIME DETECTION (Feature 19)
# ============================================================
def calculate_hurst_exponent(df: pd.DataFrame, lags: list = None) -> tuple[float, str]:
    """
    Hurst Exponent (H):
      H > 0.55 → TRENDING  (persistent, use wide TP)
      H < 0.45 → MEAN-REVERTING (anti-persistent, take profits early)
      0.45-0.55 → RANDOM WALK (avoid trading)
    """
    try:
        if lags is None:
            lags = [2, 4, 8, 16, 32]
...
# ============================================================
# GATE: KELTNER CHANNELS (Feature 7ii)
# ============================================================
def calculate_keltner_channels(df: pd.DataFrame, ema_period: int = 20, atr_mult: float = 2.0) -> dict:
    ...
    Superior to Bollinger Bands for Gold — uses ATR not std dev.
    Price above upper KC = strong trend / extended.
    Price below lower KC = strong selloff / extended.
    """
    try:
...
# ============================================================
# GATE: OBV VOLUME-PRICE DIVERGENCE (Feature 16)
# ============================================================
def check_obv_divergence(df: pd.DataFrame) -> dict:
    """
    On-Balance Volume divergence check.
    If price makes Higher High but OBV makes Lower High → BEARISH divergence.
    If price makes Lower Low but OBV makes Higher Low → BULLISH divergence.
    Filters out exhaustion moves where retail is buying the top.
    """
    try:
        if len(df) < 20:
        ...
        if price_hh and not obv_hh:
            divergence = "BEARISH"   # price up, volume not confirming
            note       = "⚠️ Price Higher High but OBV Lower High — exhaustion move"
        elif price_ll and not obv_ll:
            divergence = "BULLISH"   # price down, volume not confirming
...
# ============================================================
# GATE: LIQUIDITY SWEEP DETECTION (Feature 18)
# ============================================================
def detect_liquidity_sweep(df: pd.DataFrame) -> dict:
    """
    Detects stop hunts / liquidity sweeps:
    Equal highs swept then reversed = institutions filled short orders.
    Equal lows swept then reversed = institutions filled long orders.
    """
    try:
        if len(df) < 15:
        ...
        # Find equal highs / equal lows in prior candles
        prior     = recent.iloc[:-2]
        prev_high_max = float(prior["high"].max())
        prev_low_min  = float(prior["low"].min())

        sweep_up   = (c_high > prev_high_max and          # swept above prior high
                      c_close < prev_high_max and          # closed back below
                      (c_high - c_close) > 0.5 * (c_high - c_low))  # upper wick dominant

        sweep_down = (c_low < prev_low_min and            # swept below prior low
                      c_close > prev_low_min and           # closed back above
                      (c_close - c_low) > 0.5 * (c_high - c_low))   # lower wick dominant

        if sweep_up:
            return {
                "sweep_detected": True,
                "sweep_type":     "BEARISH_SWEEP",   # swept highs → expect down
            ...
            return {
                "sweep_detected": True,
                "sweep_type":     "BULLISH_SWEEP",   # swept lows → expect up
...
async def get_fear_greed_index() -> dict:
    """Fetch CNN Fear & Greed Index (free API)."""
    try:
...
async def get_gold_silver_ratio() -> dict:
    """
    Fetch XAG/USD and compute Gold-Silver Ratio.
    GSR > 80 → Gold overpriced relative to Silver → lower BUY confidence.
    """
    try:
...
# ============================================================
# GATE: VOLUME-WEIGHTED MACD (Feature 6i)
# ============================================================
def calculate_vw_macd(df: pd.DataFrame, fast: int = 12, slow: int = 26, signal: int = 9) -> dict:
    """
    Volume-Weighted MACD — only counts price moves backed by volume.
    Ensures BUY signals have real institutional participation.
    Falls back to regular MACD if volume data unavailable.
    """
    try:
...
# ============================================================
# GATE: ATR TRAILING STOP (Feature 15ii)
# ============================================================
async def update_trailing_stops():
    """
    ATR-based trailing stop (2.5×ATR).
...
# Wraps generate_gold_signal with all new gates
# ============================================================
async def run_full_gate_check(pair: str, indicators: dict, signal_type: str, df: pd.DataFrame) -> tuple[bool, str]:
    """
    Runs all new institutional gates IN ORDER.
    Returns (allowed: bool, reason: str).
    Call this inside generate_gold_signal BEFORE sending to AI.
    """
    ...
    # H in random walk zone — reduce confidence not block
    if 0.45 <= H <= 0.55:
    ...
    if sweep["sweep_detected"]:
        ...
        # Sweep confirms direction — log it as a boost not a block
    ...
    if gsr["extreme"] and signal_type == "BUY":
        ...
        # Soft warning — log but don't block (adjust score interpretation in AI)
