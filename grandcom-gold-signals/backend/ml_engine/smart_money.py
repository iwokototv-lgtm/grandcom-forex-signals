...
class SmartMoneyAnalyzer:
    ...
    def _calculate_premium_discount(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if current_price > equilibrium:
            zone = "PREMIUM"
            zone_strength = (current_price - equilibrium) / (range_high - equilibrium) if range_high != equilibrium else 0
        elif current_price < equilibrium:
            zone = "DISCOUNT"
            zone_strength = (equilibrium - current_price) / (equilibrium - range_low) if equilibrium != range_low else 0
        else:
            zone = "EQUILIBRIUM"
            zone_strength = 0.5
