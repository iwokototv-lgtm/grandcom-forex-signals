...
class SignalOutcomeTracker:
    ...
    async def close_signal(self, signal_id: str, outcome: Dict[str, Any], on_close_callback=None) -> bool:
        ...
        try:
            # Update in database
            update_result = await self.db.gold_signals.update_one(
            ...
            if update_result.modified_count > 0:
                ...
                signal = await self.db.gold_signals.find_one({"_id": ObjectId(signal_id)})
                if signal:
...
<i>🤖 Auto-tracked by Grandcom Gold ML Engine</i>
            """
    ...
    async def check_all_active_signals(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_signals = await self.db.gold_signals.find({
                "status": "ACTIVE"
            }).to_list(length=100)
...
# Global instance (will be initialized in gold_server.py)
